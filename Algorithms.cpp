...
string BellmanFord(Graph &G, int u, int v);
enum Color {
    WHITE,
    GRAY,
    BLACK,
...
void dfs(Graph &G, int v, vector<bool> &visited) { //simple dfs
    visited[v] = true;
    for (int i =0; i < G.getVerticesCount(); i++){             // iterating on each of the vertex neighbors
...
string Algorithms::shortestPath(Graph &G, int u, int v) { 
    ...
    vector<vector<int>> &adjacencyMatrix = G.getAdjacencyMatrix();
